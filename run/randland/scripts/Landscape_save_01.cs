...
public static class Landscape_save_01
{
	...
	static public float Continents( math.Vec2 pos )
	{
		...
		var fullerContinentMask = (float)Math.Sqrt( Math.Sqrt( Math.Sqrt( continentMask ) ) );
		...
		var flat_1_land = (float)Math.Log10( ((flat_1_clamp + 1.0f)) + 1.50f );
